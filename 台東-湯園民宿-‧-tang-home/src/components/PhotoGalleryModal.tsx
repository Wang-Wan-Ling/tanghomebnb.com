  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Photo Grid Content - Light Gray Background, Clean White Cards without Descriptions */}
            ...
            {filteredPhotos.map((photo) => {
              ...
              return (
                <div
                  ...
                  className="group relative bg-white border border-stone-200/90 rounded-none overflow-hidden hover:border-stone-400 transition-all duration-300 flex flex-col justify-between cursor-pointer hover:shadow-lg hover:-translate-y-0.5"
                >
                  {/* Photo Thumbnail */}
                  <div className="relative aspect-[16/11] overflow-hidden bg-stone-100">
                    ...
                    />
                    {photo.tag && (
                      <div className="absolute top-2.5 left-2.5 bg-stone-900/80 backdrop-blur-xs text-white text-[10px] font-mono px-2 py-0.5 rounded-none">
                        {photo.tag[currentLang]}
                      </div>
                    )}
                  ...
                  {/* Clean Caption & Category Header (Pure titles, NO descriptions) */}
                  <div className="p-3.5 flex items-center justify-between bg-white border-t border-stone-100">
                    <div>
                      <div className="text-[10px] font-mono text-[#8c6d3f] uppercase tracking-wider mb-0.5 font-medium">
                        {photo.categoryLabel[currentLang]}
                      </div>
                      <h4 className="font-serif text-sm text-stone-900 font-normal group-hover:text-[#8c6d3f] transition-colors leading-snug">
        ...
        >
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <span className="text-xs font-serif text-stone-200">
          ...
          >
            ...
            <img
              ...
              className="max-h-[78vh] max-w-full object-contain shadow-2xl animate-in zoom-in-95 duration-200"
              referrerPolicy="no-referrer"
            />
          ...
          {/* Lightbox Caption Bottom Bar (Minimalist Title, No Description) */}
          <div
            className="w-full max-w-md bg-stone-900/90 border border-stone-800 py-2.5 px-4 text-center mt-3 backdrop-blur-md"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="text-[11px] font-mono text-stone-400">
              {activePhoto.categoryLabel[currentLang]}
              {activePhoto.tag && ` ‧ ${activePhoto.tag[currentLang]}`}
            </div>
